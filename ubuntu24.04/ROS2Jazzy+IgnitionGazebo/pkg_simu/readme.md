...
2. 本项目基于ROS2 Jazzy+Ignition Gazebo
请确定你要使用的环境，自行安装ROS2
...
```
2. 按照官网教程从classic gazebo迁移到ignition gazebo
```
https://gazebosim.org/docs/harmonic/migrating_gazebo_classic_ros2_packages/
```
3. 注意事项
```
由于gazebo默认渲染引擎是ogre2,真机上一般都有物理硬件支持，但是虚拟机没有，所以需要额外加--render-engine ogre参数指定用ogre引擎  
```
4. Gazebo迁移说明  
目前按照2中的文档进行迁移，有几处错误需要注意。  
部分驱动和传感器的键值有误需要修改。请在移植之后以正式官方文档为准进行修改：
```
https://gazebosim.org/docs/harmonic/moving_robot/
https://gazebosim.org/docs/harmonic/sensors/
```
差速驱动插件中的</max_linear_acceleration>属性异常，在正式文档中已经删除，设置这个属性会导致后退异常（速度过快且似乎无法立刻停止）  
激光雷达传感器仅需修改 type="gpu_lidar">，无需修改<ray>为<lidar>，这会导致雷达扫描结果异常（只产生最小半径的结果）  
关于xml的警告可能是由于使用了ros官方推荐的urdf格式进行传感器描述而非使用gazebo官方推荐的sdf导致，gazebo可以自动处理urdf类型，因此这些警告是正常的
...
```
colcon build
source ~/Desktop/ros2_learn/py_ros2_src/ros2_pkg/simu_v1/install/setup.bash
```
4. 运行gazebo仿真
```
...
3. 关于局部代价地图  
对于humble来说，ros2bringup默认使用MPPI局部控制器
部分极端场景，比如路径上包含锐角等，对于这种情况似乎没有直观简单的补救方式，只能尽量避免直角甚至锐角转弯或使用分段导航
